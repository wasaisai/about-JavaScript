 ...
 * 4. 预解析线程解析出来一个外部theme.css文件, 并发起theme.css的下载. 这里也有一个空闲时间需要注意, 就是在DOM构建结束之后, theme.css文件还未下载完成的这段时间内, 渲染流水线无事可做, 因为下一步是合成布局树, 而合成布局树需要CSSOM和DOM, 所以这里需要等待css加载结束并解析成CSSOM
 * 5. 解析出CSSOM
 * 6. 创建布局树
...
// 显示器是怎么显示图像的
/**
 * 每个显示器都有固定的刷新频率, 通常是60HZ, 也就是每秒更新60张图片, 更新的图片都来自于显卡中一个叫前缓冲区的地方, 现实器所做的任务很简单
